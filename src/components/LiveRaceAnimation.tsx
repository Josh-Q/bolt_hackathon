...
import { Race, MLModel } from '../types/racing';
...
const LiveRaceAnimation: React.FC<LiveRaceAnimationProps> = ({ race, isActive }) => {
  ...
  useEffect(() => {
    if (!isActive || race.status !== 'running') return;
      ...
      setPositions(prev => {
        ...
        race.models.forEach(model => {
          ...
          const accuracy = model.prediction ? 
            Math.abs(race.currentPrice - model.prediction) / race.currentPrice : 0.5;
    ...
    return (
      <div className="bg-gradient-to-br from-orange-100 to-red-100 rounded-2xl p-8 border-2 border-orange-200">
        <div className="text-center text-gray-600">
          ...
          <p className="text-lg font-medium">Race will start when betting locks...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gradient-to-br from-orange-100 to-red-100 rounded-2xl p-8 border-2 border-orange-200">
      <div className="text-center mb-6">
        <h3 className="text-2xl font-bold text-gray-900 mb-2">🏁 Live Race Track 🏁</h3>
        <p className="text-orange-600">Robot Doges racing to predict the price!</p>
      </div>
                ...
                {isLeading && (
                  <div className="absolute -left-8 top-1/2 transform -translate-y-1/2">
                    <div className="flex space-x-1">
                      ...
                      <div className="w-2 h-1 bg-yellow-500 rounded animate-ping" style={{ animationDelay: '0.1s' }}></div>
                    </div>
                  </div>
                )}
        ...
        {race.models.map((model) => {
          ...
          return (
            ...
            >
              ...
              <div className="text-xs text-orange-600 mb-1">{model.personality}</div>
