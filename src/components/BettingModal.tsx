...
import { MLModel, Race } from '../types/racing';
...
interface BettingModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onPlaceBet: (modelId: string, amount: number) => void;
  userBalance: number;
}
...
const BettingModal: React.FC<BettingModalProps> = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedModel && betAmount > 0 && betAmount <= userBalance) {
      onPlaceBet(selectedModel, betAmount);
      onClose();
      setSelectedModel('');
      setBetAmount(5);
    }
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mb-6 p-4 bg-orange-50 rounded-lg border border-orange-200">
            <div className="text-center">
              <h3 className="font-bold text-gray-900 mb-2">{race.name}</h3>
              <div className="text-sm text-gray-600">
                Current Price: <span className="font-bold">${race.currentPrice.toFixed(4)}</span>
              </div>
            </div>
          </div>
              ...
              {race.models.map((model) => (
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="text-xs text-orange-600 mb-2">{model.personality}</div>
          ...
          <div className="mb-6">
            ...
            <div className="text-sm text-gray-600 mt-2">
              Balance: {userBalance.toFixed(2)} DOGE
            </div>
          </div>
          ...
          {selectedModel && betAmount > 0 && (
            <div className="mb-6 p-4 bg-green-50 rounded-lg border border-green-200">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-gray-700">Potential Payout:</span>
                <span className="text-xl font-bold text-green-700">
                  {calculatePayout().toFixed(2)} DOGE
                </span>
              </div>
              <div className="text-sm text-green-600 mt-1">
                3x your bet if your AI wins!
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              type="submit"
              disabled={!selectedModel || betAmount <= 0 || betAmount > userBalance}
